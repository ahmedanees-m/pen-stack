"""Build-interface unit tests: protocol export, gated ingestion, simulated lab.

CI-safe. Asserts the digital→physical bridge:
  * a cleared, legal design exports a valid protocol DRAFT (>=1 platform) with the immune profile in the
  ...
  * ingestion is typed + gated: a result becomes a quarantined measured Candidate, admitted to the curated
    world-model ONLY through the gate (no auto-edit); a malformed result raises;
...
def test_cleared_design_exports_draft_with_immune_metadata():
    for target in ("opentrons", "pylabrobot", "cloudlab"):
        ...
        assert "immune_profile" in code # immune profile travels with the protocol
...
def test_admission_only_via_gate():
    ...
    # with approval -> routed through the gate; a measured, provenance-bearing edge is admitted
