"""Oracle mesh contract + adapter unit tests. Pure-logic / contract-level, CI-safe
...
def test_evo2_live_falls_back_to_deferred_when_hosted_call_fails(monkeypatch):
    # a hosted failure must degrade to a deferred CANDIDATE, never a fabricated sequence
    monkeypatch.setenv("PEN_STACK_ORACLE_NET", "1")
    monkeypatch.setenv("NVIDIA_API_KEY", "test-key")
    def _boom(seed, n):
...
def test_live_oracle_status_surface(monkeypatch):
    # the execution/latency map covers every wrapped model and the status reflects the live conditions
    monkeypatch.delenv("PEN_STACK_ORACLE_NET", raising=False)
    ...
    assert st["state"]["execution"] == "deferred" and st["state"]["live"] is False # deferred
    s = summary()
...
def test_hosted_oracle_goes_live_when_enabled(monkeypatch):
    ...
    # AlphaGenome additionally requires its client package (live check): live IFF the package is
