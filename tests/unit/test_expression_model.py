"""The comprehensive, literature-cited relative-expression model + its validation.
...
anchor (Qin 2010): promoter strength is context-dependent. These tests lock both the model and that context-dependence.
"""
...
def test_modifier_profile_is_a_bounded_range_not_a_point_multiplier():
    ...
    assert mp["estimated_uplift_range"][0] == 1.0 # lower bound: may do nothing for a transgene
...
def test_expression_proxy_does_NOT_falsely_claim_validation():
    ...
    assert res["status"] != "outcome_validated" # it does NOT validate cross-study
