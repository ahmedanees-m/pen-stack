"""Genome-Writing Challenge + co-scientist unit tests.
...
def test_reference_anchors_and_round_is_well_formed():
    ...
    assert r["immune_risk_task_included"] is True # the grounded immune-risk task is present
...
def test_co_scientist_drives_full_loop_immune_first_class():
    ...
    assert s["strategies"] # Pareto frontier
    assert all("interval" in o for o in s["predicted_outcomes"]) # calibrated outcomes
    assert s["suggested_experiments"] # experiments
    assert set(s["safety"]) <= {"clear", "flag"} # all cleared/flagged
    assert "assessed" in s["scope_ledger"] # scope ledger
...
def test_co_scientist_immune_profiles_present_and_uncollapsed():
    ...
    for p in s["immune_profiles"]:
        ...
        assert p["collapsed_score"] is None # invariant preserved end-to-end
