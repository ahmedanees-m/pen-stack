"""Closed-loop unit tests (gated DBTL loop, drift detection, continual learning; autonomy Level 3).
...
CI-safe (explicit candidate pool + sim-lab). Asserts:
  ...
  * the convergence (active-vs-random) is reported with a CI either way (falsifiable), Level 3 not 5.
"""
...
def test_convergence_reported_with_ci_falsifiable():
    ...
    assert isinstance(c["reaches_optimum_faster_than_random"], bool) # reported either way
