"""Release meta-tests: the release artifacts are present and consistent."""
...
def test_version_is_8_0_5_everywhere():
    assert pen_stack.__version__ == "8.0.5"
    assert 'version = "8.0.5"' in (_ROOT / "pyproject.toml").read_text(encoding="utf-8")
    assert "version: 8.0.5" in (_ROOT / "CITATION.cff").read_text(encoding="utf-8")
    assert "[8.0.5]" in (_ROOT / "CHANGELOG.md").read_text(encoding="utf-8")
    assert 'org.opencontainers.image.version="8.0.5"' in (_ROOT / "Dockerfile").read_text(encoding="utf-8")
...
def test_v7_0_artifacts():
    # the v7.0 capstone: closed loop (cloud-lab connector + SDL-brain benchmark + validation campaign)
...
def test_v6_14_artifacts():
    # the v6.14 artifacts: WriteSpec (typed SBOL3-profile intent layer + grounded extractor + bench + SAT)
...
def test_v6_11_artifacts():
    # the v6.11 artifacts: cross-modality deliverability + learned FLIP-AAV capsid-fitness
...
def test_v6_10_artifacts():
    # the v6.10 artifacts: cross-family off-target nomination + Off-Target-Bench; v6.10.1 = 4-assay
    # expansion (real CRISOT > homology on all four)
    ...
    cl = (_ROOT / "CHANGELOG.md").read_text(encoding="utf-8")
    assert all(f"[6.10.{n}]" in cl for n in range(5))
    ...
    # v6.10.3: chromatin validated (moderate, cell-type-matched); v6.10.4: incremental over CRISOT tested ->
    # annotation, not a re-ranker (does not change the numeric risk score)
...
def test_v6_9_artifacts():
    # the v6.9 artifacts: MHC-II/CD4 + ADA + writer-as-antigen; v6.9.1 = REAL NetMHCIIpan cache;
    # v6.9.2 = real-tool rigor pass (NetMHCpan-4.1 capsid MHC-I, real human-proteome self-match, real Bxb1 attB)
    ...
    cl = (_ROOT / "CHANGELOG.md").read_text(encoding="utf-8")
    assert "[6.9.0] -" in cl and "[6.9.1] -" in cl and "[6.9.2] -" in cl
    ...
    # v6.9.2: the real human-proteome self-match discriminates self (albumin 1.0) from foreign (Cas9 0.0)
...
def test_v6_8_artifacts():
    # the v6.8 artifacts: curated writer-efficiency dataset + bench + predictor + guide/variant
...
def test_v6_7_artifacts():
    # the v6.7 artifacts: learned, trained-conformal position-effect model + TPE-Bench
...
def test_v6_2_artifacts():
    # the v6.2 artifacts: the Web Platform (grounded co-scientist + UX frontend + Docker self-host + preregs)
...
def test_changelog_has_5_13_0_entry():
    cl = (_ROOT / "CHANGELOG.md").read_text(encoding="utf-8")
    assert "[5.13.0] -" in cl and "[5.12.0] -" in cl
