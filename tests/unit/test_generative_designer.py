"""Generative-designer unit tests: verifier-as-discriminator, Pareto frontier, orchestration.
...
needed; the planner-backed candidate_space is the data/VM path). Audit writes go to a tmp file. Asserts:
  ...
  * survivors are legal + safe, each output_kind="candidate" with a calibrated confidence + an immune profile;
  * Pareto frontier is non-dominated and the neg_immune_risk axis is GROUNDED by the profile (uncertainty
...
def test_survivors_are_candidates_with_confidence_and_immune_profile():
    for s in _survivors():
        ...
        assert s["immune_profile"]["collapsed_score"] is None # invariant preserved
...
def test_candidate_space_graceful_without_atlas():
    ...
    # with the writability atlas absent (CI), plan_write yields nothing -> [] (never crashes)
    out = candidate_space({"gene": "AAVS1", "intent": "safe_harbour_insertion", "cargo_bp": 3000})
    assert isinstance(out, list)


# --- Grounded immune-risk axis + non-dominance ------------------------------
...
def test_neg_immune_risk_is_grounded_not_placeholder():
    ...
    # value equals the worst-case in-scope axis value from the profile (sourced, not constant)
