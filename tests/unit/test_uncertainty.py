"""Uncertainty-quantification unit tests: pure-logic, synthetic data, run everywhere (no real atlas/VM needed).
...
def test_conformal_quantile_finite_sample():
    ...
    assert 0.85 <= q <= 0.95
    # too few points to certify -> +inf (not a falsely tight bound)
...
def test_risk_coverage_flat_when_confidence_uninformative():
    ...
    rc = risk_coverage_curve(confidence, correct)
    # a NEGATIVE result: uninformative confidence does not strictly improve accuracy
...
def test_structure3d_confidence():
