"""WT-KB schema + coverage tests (pre-registered success criteria)."""
...
def test_wtkb_builds_and_validates():
    ...
    assert len(df) >= 6, ">=6 fully-specified writer families"
...
def test_at_least_two_tier1_families_for_reachability():
    # Reachability needs Tier-1 sites recoverable for >=2 families
    df = build(str(CURATED))
