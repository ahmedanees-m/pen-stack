"""CD4/MHC-II + ADA + writer-as-antigen.
...
writer-as-antigen dominant-risk flag, and that the ADA axis stays a proxy (no manufactured ).
"""
...
# ---- REAL NetMHCIIpan-4.0 MHC-II + no production proxy ----------------------------------
def test_mhc2_real_cache_is_grounded_and_bounded():
    # the production MHC-II axis is the REAL NetMHCIIpan-4.0 residue-coverage cache (by antigen name)
...
def test_mhc2_abstains_without_a_grounded_name_no_production_proxy():
    # Honesty: an uncached sequence (no name) is a KNOWN-UNKNOWN, it ABSTAINS, never a guessed
...
def test_real_netmhciipan_cache_wired_and_discriminates():
    ...
    assert mhc2_epitope_load(s)["backend"] == "abstain" # NO production proxy, abstains
...
# ---- ADA-risk (real MHC-II x origin-foreignness) + real self-match cross-check ----------
...
def test_writer_immunogenicity_table_surfaces_self_control():
    # the Writer Atlas immunogenicity table surfaces the human self control (albumin) as a labelled
    # reference row, so the ADA formula's multiplicative work is VISIBLE (not just unit-tested): a self protein
    # carries MHC-II epitopes (density > 0) yet ADA-risk is 0 (origin=self -> foreignness 0), decoupling ADA from
    # the MHC-II load. Without a non-foreign row the axis reads like a pass-through of the load.
    from pen_stack.planner.immune_profile import writer_immunogenicity_table
    rows = writer_immunogenicity_table()
    ctrl = [r for r in rows if r.get("is_control")]
    writers = [r for r in rows if not r.get("is_control")]
    assert len(ctrl) == 1 and ctrl[0]["representative"] == "HumanAlbumin"
    c = ctrl[0]
    assert c["origin"] == "self" and c["foreignness"] == 0.0
    assert c["epitope_density"] > 0.0 and c["ada_risk_score"] == 0.0  # epitopes present, yet tolerated
    # the genome writers are foreign and DO score ADA risk > 0, the foreignness multiplier is not a no-op
    assert writers and all(w["foreignness"] == 1.0 and w["ada_risk_score"] > 0.0 for w in writers)


def test_ada_abstains_when_origin_unknown_no_proxy():
    # foreignness is the protein ORIGIN (authoritative central-tolerance signal). An UNKNOWN origin
...
# ---- Never-collapsed profile + writer-as-antigen ---------------------------------------
def test_profile_has_mhc2_ada_axes_never_collapsed():
    ...
    assert "proxy" in p["axes"]["mhc2_writer"]["validation"].lower() # label travels
...
# ---- Bench recovery + ADA calibration -------------------------------------------
def test_immuno_bench_recovery_and_calibration():
