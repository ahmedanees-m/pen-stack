"""Epistemic status + known-unknowns scope (pure-logic, CI-safe)."""
...
def test_pen_agent_tags_steps_with_epistemic():
    # runs without the writability atlas -> site step refuses, but every step still carries an epistemic verdict
    from pen_stack.agent.pen_agent import plan_write_session
    r = plan_write_session("AAVS1", "safe_harbour_insertion")
