"""Calibrate immune-risk PROXIES against observed outcomes.
...
when there are too few points. And the central proxy-labelling gate: every CURRENT real axis is labelled a proxy (no
...
def test_every_current_axis_is_labelled_a_proxy():
    # the central proxy-labelling gate: with no sufficient public outcome dataset, NO real axis is
    ...
    assert set(AXIS_STATUS) == {"genotoxicity", "cd8_epitope", "innate", "preexisting_nab", "anti_peg",
                                "mhc2_writer", "ada_writer"} # CD4/MHC-II + ADA writer axes
...
def test_run_report_is_two_sided():
