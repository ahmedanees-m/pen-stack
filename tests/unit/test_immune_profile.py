...
def test_writer_immunogenicity_table_for_the_writer_atlas():
    # v7.1.8 + v7.3.7: the writer-as-antigen immunogenicity (MHC-II + ADA) is surfaced on the Writer Atlas, read
    # from the committed NetMHCIIpan-4.0 cache (no recompute). It covers the genome-writer families (serine
    # integrase Bxb1, bridge ISCro4), EXCLUDES the Cas9 nuclease (an editor), and now (v7.3.7) surfaces the human
    ...
    # v7.3.7: the human self control IS surfaced, origin self, foreignness 0 -> ada_risk 0 despite MHC-II epitopes
