"""Unified per-design immune-risk profile (never collapsed).
...
report None (not a guess); verify() exposes it; the route modifier + new known-unknowns are present."""
...
_AXES = {"genotoxicity", "cd8_epitope", "innate", "preexisting_nab", "anti_peg",
         "mhc2_writer", "ada_writer"} # CD4/MHC-II + ADA over the writer enzyme
...
def test_profile_returns_all_axes_with_uncertainty_and_label():
    ...
    for axis, rec in p["axes"].items():
        ...
        assert "proxy" in rec["validation"].lower() # calibration labels travel with the profile


def test_innate_axis_derives_cargo_form_from_vehicle():
    # when a cargo sequence is supplied but no explicit cargo_form, the form is derived from the vehicle's
    ...
    # DNA vehicle + cargo_seq, no explicit cargo_form -> innate now computes (was abstaining before).
...
def test_writer_axes_compute_when_a_writer_is_supplied():
    # the MHC-II/CD4 + ADA writer-as-antigen axes compute when a grounded writer family is named (bundled
...
def test_writer_immunogenicity_table_for_the_writer_atlas():
    # the writer-as-antigen immunogenicity (MHC-II + ADA) is surfaced on the Writer Atlas, read
    # from the committed NetMHCIIpan-4.0 cache (no recompute). It covers the genome-writer families (serine
    # integrase Bxb1, bridge ISCro4), EXCLUDES the Cas9 nuclease (an editor), and now surfaces the human
    # self control (albumin) as a labelled reference row so the ADA foreignness multiplier is visible.
    ...
    assert "SpCas9" not in reps  # Cas9 removed (an editor, not a large-cargo writer)
    writers = [w for w in t if not w.get("is_control")]
    controls = [w for w in t if w.get("is_control")]
    assert {w["writer_family"] for w in writers} == {"serine_integrase", "bridge_IS110"}
    for w in writers:
    ...
    # the human self control IS surfaced, origin self, foreignness 0 -> ada_risk 0 despite MHC-II epitopes
    assert len(controls) == 1 and controls[0]["representative"] == "HumanAlbumin"
    assert controls[0]["is_foreign"] is False and controls[0]["ada_risk_score"] == 0.0


def test_administration_context_mutes_vector_facing_axes_ex_vivo():
    # ex-vivo administration (cells transduced in a dish, washed before transplant) bypasses the patient's
...
def test_profile_is_never_collapsed_into_one_number():
    # the central gate: no single fused score (that would fake confidence).
...
def test_known_unknowns_listed_and_magnitude_never_predicted():
    ...
    # the mechanistically-distinct axes are registered as known-unknowns
