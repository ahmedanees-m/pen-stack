"""Plan-confidence calibrated against documented outcomes.
...
def test_interpretation_states_calibration_caveat():
    r = _report()
    # high ECE + significant gap must be reported as useful-for-ranking, not as absolute calibration
    assert r["ece"] > r["prevalence"] # confidence over-states absolute recovery
