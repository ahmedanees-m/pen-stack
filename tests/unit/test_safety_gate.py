"""Biosecurity / dual-use screening gate (the Guardian).
...
def test_empty_submission_is_distinguished_from_a_screened_clear():
    """An empty Cargo function box (no function/domain/taxon signal at all) must not read identically to a
    genuinely-screened benign design -- both decide "clear", but only the screened one earned it."""
    empty = safety_gate({"gene": "AAVS1", "cargo_function": ""}, actor="test")
    assert empty.decision == "clear"
    assert empty.provenance["declared_signal"] is False
    assert "nothing was screened" in empty.reason.lower()

    benign = safety_gate({"cargo_function": "human factor IX"}, actor="test")
    assert benign.decision == "clear"
    assert benign.provenance["declared_signal"] is True
    assert "nothing was screened" not in benign.reason.lower()
...
# --- CRITICAL FIXES: viral tropism + oncogenic functions ----------------------

def test_furin_cleavage_enhancement_caught():
    """CRITICAL: Guardian must catch furin-cleavage enhancement (viral tropism GOF)."""
...
def test_dominant_negative_tp53_caught():
    """CRITICAL: Guardian must catch dominant-negative TP53 (oncogenic GOF)."""
...
# --- oncogenic-manipulation PATTERN screen (mechanism/synonym-robust, red-team-derived) ----------
...
def test_short_toxin_keyword_no_substring_false_positive():
    """the ricin abbreviation 'rip' must NOT fire inside 'transcription' (word-boundary match)."""
