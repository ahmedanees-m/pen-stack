"""Genome-Writing Bench + PEN-Agent. Pure-logic, CI-safe (no atlas/Perry/LLM required).
...
def test_tasks_have_deterministic_scorers_and_no_circular_labels():
    ...
    for t in cfg["tasks"]:
        assert t["circular"] is False # non-circularity gate inherited
...
def test_pen_agent_never_fabricates_without_atlas():
    # No writability atlas in CI -> site selection refuses; the agent must NOT raise and must NOT fabricate.
...
def test_leaderboard_renders_real_llm_agent_row():
    ...
    # a REAL LLM-agent run carries an llm_agent detail block -> the row is labeled llm_agent + a note
...
def test_ungrounded_classifier_separates_fabrication_from_refusal():
    # The fabrication classifier must flag concrete tool-only numbers as fabrication and must not flag refusals as fabrication.
