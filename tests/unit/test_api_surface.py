"""AI Integration Surface unit tests (manifests, framework tool wrapper, REST + MCP endpoints).
...
def test_capability_manifest_is_typed():
...
def test_scope_manifest_exposes_known_unknowns_and_scope_cards():
    ...
    # the in-vivo / phenotype known-unknowns are present (the scope boundary)
...
# --- REST + MCP endpoints (VM / server extra) ---------------------------------
...
def test_count_params_reject_non_positive():
    # New finding: k / top / top_k / limit accepted negatives -> Python negative-slice artifacts (k=-1 returned
    # 134 plans instead of 5). A lower bound (ge=1) must 422 them at validation, BEFORE the handler runs, so this
    # is data-independent (no atlas/coords needed). Required params are supplied so only the count is invalid.
    pytest.importorskip("fastapi")
    ...
    c = TestClient(app)
    cases = [
        ("/plan", {"gene": "CCR5", "intent": "safe_harbour_insertion", "k": -1}),
        ("/writable", {"gene": "CCR5", "top": -1}),
        ("/writable", {"gene": "CCR5", "top": 0}),
        ("/atlas", {"limit": -5}),
        ("/recommend", {"top_k": -1}),
        ("/recommend", {"cargo_bp": 0}),        # cargo_bp bound also on /recommend, not just /plan
        ("/recommend", {"cargo_bp": -100}),
        ("/recommend", {"cargo_bp": 999999999}),  # absurd payload (> human genome) rejected, not a 200
        ("/plan", {"gene": "CCR5", "intent": "safe_harbour_insertion", "cargo_bp": -100}),
        ("/crosslink/loci", {"family": "bridge_IS110", "top": -2}),
    ]
    for path, params in cases:
        assert c.get(path, params=params).status_code == 422, (path, params)
    # a valid positive count still validates (proves ge=1 didn't over-reject the default range)
    assert c.get("/atlas", params={"limit": 5}).status_code == 200


def test_recommend_validates_write_type():
    # write_type is validated now (was: any arbitrary string accepted and silently ignored). Known write types
    # pass; unknown/garbage 422s at the endpoint.
    pytest.importorskip("fastapi")
    pytest.importorskip("httpx")
    from fastapi.testclient import TestClient
    ...
    from pen_stack.server.api import app
    c = TestClient(app)
    assert c.get("/recommend", params={"write_type": "insertion"}).status_code == 200
    assert c.get("/recommend", params={"write_type": "repeat_excision"}).status_code == 200
    assert c.get("/recommend", params={"write_type": "foobar"}).status_code == 422
    assert c.get("/recommend", params={"write_type": "garbage_value"}).status_code == 422


def test_health_reports_writability_atlas_presence():
    """The health check must surface whether the per-cell-type WRITABILITY atlases (mounted at runtime, not
    bundled) are present, so a deploy that drops that mount is visible instead of silently green. CI has no
    atlas parquets, so we assert the SHAPE (a measured_count + ok bool), not a nonzero count."""
    pytest.importorskip("fastapi")
    pytest.importorskip("httpx")
    from fastapi.testclient import TestClient
    ...
    c = TestClient(app)
    h = c.get("/health").json()
    assert h["status"] == "ok" and "atlas_present" in h
    wa = h["writability_atlas"]
    assert set(wa) == {"measured_count", "measured", "ok"}
    assert isinstance(wa["measured_count"], int) and isinstance(wa["measured"], list)
    assert wa["ok"] == (wa["measured_count"] > 0)
