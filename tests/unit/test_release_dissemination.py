"""Release + dissemination. Meta-tests that the release artifacts are consistent."""
...
def test_version_consistent_everywhere():
    # the single released version is consistent across __init__ == pyproject == CITATION
...
def test_readme_mentions_benchmark_and_no_stale_version():
    r = (_ROOT / "README.md").read_text(encoding="utf-8")
    assert "Genome-Writing Bench" in r
    assert "8.1.2" not in r # no stale prior-release version string left
...
def test_pypi_packaging_ready():
    ...
    assert "Development Status :: 5 - Production/Stable" in pp and "Issues =" in pp # production-stable release
