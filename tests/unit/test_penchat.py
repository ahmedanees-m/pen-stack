"""The grounded chat General lane (RAG).

Runs with the deterministic LEXICAL retriever (PEN_RAG_NO_EMBED=1) so it is reproducible in CI without Ollama;
the committed corpus + embeddings still ship for the live semantic path. Asserts the properties:
...
def test_provider_abstraction_is_documented_and_swappable(monkeypatch):
    # a clean provider interface with >=2 providers and a documented default; NO_LLM disables it.
...
def test_general_lane_uses_fast_path_timeout_and_token_cap(monkeypatch):
    # latency fix: kind="general" picks a tighter timeout / token cap than the default (engine-grounded) path.
...
def test_query_embedding_is_cached(monkeypatch):
    # latency fix: a repeated query phrase reuses the cached embedding instead of re-calling Ollama.
...
def test_grounded_fields_are_provider_independent_by_construction(monkeypatch):
    # the acceptance invariant: the grounded fields - lane, provenance, cited sources - come from retrieval
...
def test_followup_to_grounded_answer_stays_grounded():
    # memory carries the prior lane; a back-reference follow-up to a GROUNDED answer routes to the grounded
...
def test_routing_benchmark_meets_safety_gate():
    # a write/result request must never leak to the ungrounded general lane.
...
def test_groundedness_benchmark_meets_gates():
    # (re-scoped): citation coverage on the CITED answers, 0 unsupported, 0 false-grounding,
...
def test_safety_benchmark_meets_gates():
    # (re-scoped): false-grounding is THE headline; general questions are answered;
...
def test_headtohead_result_reframed_punchline():
    # punchline (committed LIVE result; re-framed): all three answer general questions, but only
    # The chat labels provenance and redirects (instead of fabricating a statistic) on a specific unsourceable claim.
    import json
    import pathlib
    ...
    assert s["pen_chat"]["provenance_labelled_rate"] >= 0.99   # the chat labels every answer
...
def test_chat_general_lane_answers_or_abstains_never_pen_stack(monkeypatch):
    """Through the web chat: a general query is answered (general / literature-cited) or abstains, but its
