"""Genome-wide enumeration + the nuclease finder.
...
def test_casoffinder_parser_extracts_coords_from_verbose_header():
    ...
    # a bulge/gap row (matched DNA contains '-') is skipped (Cas-OFFinder v2.0 enumerates substitutions only)
...
def test_finder_abstains_on_a_novel_guide():
    r = enumerate_offtargets("ACGTACGTACGTACGTACGTAGG", "SpCas9")
    assert r["abstain"] and r["source"] == "none"
    assert "runs on the VM" in r["note"]  # VM/cache pointer, no fabricated sites
...
def test_dispatcher_is_finder_by_default_and_scorer_when_candidates_supplied():
    ...
    assert scr["available"] and "mode" not in scr  # backward-compatible scorer path


def test_cast_untargeted_background_per_system():
    # CAST off-target = guide-independent untargeted transposition (per-system) + guide-directed. Type V-K
    # (ShCAST) is HIGH untargeted; Type I-F (VchCAST) is LOW. All documented, DOI-tagged, mechanism-unvalidated
    # (deliberately hard-locked: no genome-wide unbiased CELLULAR site assay exists, the documented-limitation call).
...
def test_bridge_hard_locked_unvalidated_with_disclosure():
    # bridge is hard-locked mechanism_based_unvalidated (no genome-wide cellular assay), but the ranker is
...
def test_integrase_phic31_encoded_with_sealed_negative_recall_benchmark():
    # PhiC31 is now ENCODED (verified GenBank pseudo-attP AF333429/30/31 + PDB 9U2T/9U2S att). The sealed
    # recall benchmark is NEGATIVE, att-similarity does not recover the documented sites above background, and is
    # reported verbatim (a stronger outcome than "semi-validated"). No fabrication.
    ...
    assert p["sealed_recall_benchmark"]["recovered_above_background"] is False  # negative result
    assert p["similarity_ranking_validated"] is False and p["documented_doi"]
    # precise capability disclosure, the negative is about a METHOD, not "pseudosites are unpredictable"
...
def test_integrase_bxb1_pseudo_attp_finder_reflects_high_specificity():
    # the Bxb1 genome-wide pseudo-attP scan replays the VM cache. Bxb1 is highly specific, so the
    # scan returns very few candidates (the validated property), each with a coordinate + att similarity.
    from pen_stack.wgenome.offtarget_integrase import nominate_integrase
...
def test_paste_composes_nuclease_and_integrase_components():
    # PASTE = compose(nuclease nickase, integrase att). Both components returned, each with its own status.
...
@pytest.mark.skipif(not _have_cache, reason="enumeration cache absent")
def test_enumeration_recovers_documented_emx1_offtargets():
    # enumeration reproduces the documented Cas9 off-target set (the finder's genome-wide search step).
    ...
    assert recall >= 0.90, f"recall {recall:.3f} < 0.90"  # observed 1.000
