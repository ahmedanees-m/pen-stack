"""The learned, trained-conformal position-effect model.
...
outcome integration WITHOUT the gitignored TRIP data or model artifact. The REAL-data claim (the factored model
beats the prior durability head on TRIP) runs only when the data is present (VM / local checkout), and is skipped,
not faked, in CI.
"""
...
def test_blocked_splits_are_leakage_clean_and_celltype_is_data_gated():
    ...
    assert ped.heldout_celltype_splits(df) == [] # single cell type -> data-gated


# ---- the learned model + conformal --------------------------------------------------
...
def test_stage_h_heuristic_fallback_when_no_context():
    ...
    lo, hi = o["interval"] # the interval contract is intact
    assert lo <= o["predicted_outcome"]["relative_expression"] <= hi


def test_stage_h_learned_path_with_injected_model(monkeypatch):
    """With a chromatin context AND a model present, the learned path serves a TRAINED-CONFORMAL interval + p_silenced."""
    ...
    # p_silenced must carry the not-validated flag (the silencing classifier is chance on the human head)
    assert any("silencing_classifier_not_validated" in f for f in pe["scope_flags"])
    assert "stage_h_learned_trained_conformal" in o["scope_flags"]


def test_stage_h_surfaces_a_wellformed_ood_widen_field(monkeypatch):
    """Regression (tester finding, App_testing_debugging.txt): the tester could not surface ANY OOD
    signal that reacts to the chromatin marks, because the Twin UI's top-level 'OOD' stat reflects the UNRELATED
    virtual-cell response oracle's scope card, while THIS model's own OOD signal, `position_effect.ood_widen`,
    was computed but never surfaced in the UI. This guards the CONTRACT the UI now depends on: predict_stage_h's
    output carries a well-formed `ood_widen` (a float on the widen scale, >= the 1.0 floor) for both a typical and
    an extreme context, and it is monotone non-decreasing with distance-from-training (the training mean is the
    minimum-distance point, so an extreme point never widens LESS). The separation power + strict monotonicity of
    the widen factor itself is the OODDetector's own invariant, tested in test_ws_uq (kept version-stable there;
    exact widen magnitudes through this synth-fit chain are numpy/BLAS-version-sensitive and not asserted here)."""
    from pen_stack.twin import outcome as out
    from pen_stack.twin import position_effect as pemod
    model = pemod.PositionEffectModel().fit(_synth())
    rep = pemod.evaluate(_synth())
    ...
    # the training MEAN is the guaranteed minimum-Mahalanobis-distance point; an all-50 point is far outside it.
    center = {f: float(m) for f, m in zip(model.features, model.ood.mean_)}
    extreme = {f: 50.0 for f in model.features}

    w_center = out.predict_outcome({**_DESIGN, "cassette": "promA", "chromatin_features": center}, "k562")["position_effect"]["ood_widen"]
    w_extreme = out.predict_outcome({**_DESIGN, "cassette": "promA", "chromatin_features": extreme}, "k562")["position_effect"]["ood_widen"]

    # the field the UI surfaces is present and well-formed for both contexts (the actual regression)
    for w in (w_center, w_extreme):
        assert isinstance(w, float) and w >= 1.0  # widen is a multiplier on [1, cap]
    # monotone non-decreasing: the extreme (far-from-training) point never widens LESS than the training centre
    assert w_extreme >= w_center
...
def test_tpe_bench_split_is_sealed_and_sha_locked():
    ...
    assert j["celltype_holdout"]["status"] == "data_gated" # transfer gating
...
def test_real_trip_factored_beats_durability_head_when_available():
    ...
    # the headline: factored model beats the context-only durability head, CI excludes zero
