...
data-derived score via the OracleResult contract; non-integrating vectors are 1.0 by mechanism; the
oracle abstains (never fabricates) when it has no computed class; the ROBUST integrating classes are enriched
near oncogenes from data and the lentiviral score validates the documented tier; the in-vivo clonal OUTCOME
stays a known-unknown.

Honesty note: with the license-clean CancerMine (CC0) oncogene set (broader than COSMIC CGC) the per-class
...
def test_robust_integrating_classes_are_enriched_near_oncogenes_from_data():
    ...
    # the gammaretroviral comparator is small-n (flagged directional-only); with the broad CC0 oncogene
...
def test_computed_score_validates_the_documented_tier():
    # documented lentivirus genotoxicity = "moderate" -> tier score 0.5; the COMPUTED score should
