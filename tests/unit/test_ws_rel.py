...
def test_version_is_7_1_8_everywhere():
    assert pen_stack.__version__ == "7.1.8"
    assert 'version = "7.1.8"' in (_ROOT / "pyproject.toml").read_text(encoding="utf-8")
    assert "version: 7.1.8" in (_ROOT / "CITATION.cff").read_text(encoding="utf-8")
    assert "[7.1.8]" in (_ROOT / "CHANGELOG.md").read_text(encoding="utf-8")
