...
def test_version_is_7_1_7_everywhere():
    assert pen_stack.__version__ == "7.1.7"
    assert 'version = "7.1.7"' in (_ROOT / "pyproject.toml").read_text(encoding="utf-8")
    assert "version: 7.1.7" in (_ROOT / "CITATION.cff").read_text(encoding="utf-8")
    assert "[7.1.7]" in (_ROOT / "CHANGELOG.md").read_text(encoding="utf-8")
...
def test_v6_9_artifacts():
    # the v6.9 artifacts: PEN-IMMUNE MHC-II/CD4 + ADA + writer-as-antigen; v6.9.1 = REAL NetMHCIIpan cache;
...
def test_v6_8_artifacts():
    # the v6.8 artifacts: PEN-WRITER curated writer-efficiency dataset + bench + predictor + guide/variant
...
def test_v6_7_artifacts():
    # the v6.7 artifacts: PEN-EXPRESS learned, trained-conformal position-effect model behind Stage H + TPE-Bench
