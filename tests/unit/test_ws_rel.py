...
def test_version_is_7_2_3_everywhere():
    assert pen_stack.__version__ == "7.2.3"
    assert 'version = "7.2.3"' in (_ROOT / "pyproject.toml").read_text(encoding="utf-8")
    assert "version: 7.2.3" in (_ROOT / "CITATION.cff").read_text(encoding="utf-8")
    assert "[7.2.3]" in (_ROOT / "CHANGELOG.md").read_text(encoding="utf-8")
