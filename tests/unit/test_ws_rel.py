...
def test_version_is_4_0_2_everywhere():
    assert pen_stack.__version__ == "4.0.2"
    assert 'version = "4.0.2"' in (_ROOT / "pyproject.toml").read_text(encoding="utf-8")
    assert "version: 4.0.2" in (_ROOT / "CITATION.cff").read_text(encoding="utf-8")
    assert "version-4.0.2" in (_ROOT / "README.md").read_text(encoding="utf-8")


def test_changelog_has_4_0_2_entry():
    cl = (_ROOT / "CHANGELOG.md").read_text(encoding="utf-8")
    assert "[4.0.2] -" in cl and "[4.0.1] -" in cl       # both kept
