"""Writer Atlas <-> Writable Genome cross-link.
...
Skips cleanly when the writability atlas (fetched-not-committed) is absent.
"""
...
pytestmark = pytest.mark.skipif(
    ...
    reason="writability atlas absent - run 'make fetch' (scripts/fetch_artifacts.sh) to download it "
           "from Zenodo (into data/out/); the science does not execute without it")
