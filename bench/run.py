"""Genome-Writing Bench v0.1 - one-command entrypoint.
...
benchmarks/genome_writing_bench/LEADERBOARD.md. Tasks needing the atlas / Perry tables / an LLM are
...
_FROZEN = [
    ...
    "configs/safety/probes.yaml", # the Guardian: frozen hazard/benign/red-team probe set
    "configs/safety/hazard_registry.yaml", # version-pinned hazard signatures
    "configs/safety/policy.yaml", # severity->decision policy
]
...
def write_shasums() -> Path:
    # Standard `sha256sum` format: hash + two spaces + path, LF only, so `sha256sum -c SHA256SUMS` verifies it
    # and re-sealing an unchanged tree is byte-identical (no working-tree churn).
...
def verify_shasums() -> dict:
    if not _SHA.exists():
        return {"verified": False, "reason": "SHA256SUMS missing - run with --seal first"}
    want = {}
    for line in _SHA.read_text(encoding="utf-8").splitlines():
        if line.strip():
            # Tolerant of one- or two-space separators and CRLF (standard sha256sum output uses two spaces);
            # split on any whitespace run so the parsed path never carries a leading space.
...
def _llm_orchestrator() -> dict | None:
    ...
    when an LLM provider is reachable; this is the leaderboard's >=1 external-LLM-agent baseline."""
...
def main(argv: list[str] | None = None) -> int:
    ...
    ap.add_argument("--seal", action="store_true",
                    help="re-seal the frozen SHA256SUMS from the current tree (maintainers only)")
    ...
    # The frozen SHA256SUMS is a committed integrity reference, not a run output: only re-seal it on explicit
    # --seal, so `make repro` / --agent verify against it without rewriting it (no working-tree churn).
    if args.seal:
        write_shasums()
