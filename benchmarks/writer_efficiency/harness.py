...
Baselines: the KB family-mean prior (what the curated KB ranking implies) vs the learned predictor.
Result: the learned model beats the KB baseline on held-out LOCUS (CI excludes 0) but NOT on
...
def baseline_leaderboard() -> dict:
    """Reproduce the baseline leaderboard (KB family-mean vs the learned model) on both held-out axes."""
