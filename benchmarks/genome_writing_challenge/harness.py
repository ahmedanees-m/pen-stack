"""The Genome-Writing Challenge, an open, recurring, held-out benchmark.
...
internal bench: legality, write-type, safety, and an IMMUNE-RISK task drawing on the oracles.
...
def _highest_immune_risk_axis(design: dict) -> str | None:
    """The in-scope immune axis with the LOWEST score = highest risk (from the profile)."""
...
def _pub(task_id: str, family: str, design: dict) -> dict:
    return {"task_id": task_id, "family": family, "design": design,
            "instructions": {"legality": "return True/False: is this design legal?",
                             ...
                             "immune_risk": "return the name of the highest-risk immune axis (from the axes)"
                             }[family]}


def _offtarget_instance() -> dict | None:
    """A held-out off-target NOMINATION instance from the Off-Target-Bench fixture: a guide + a candidate
...
def _round_tasks(round_id: str) -> list[ChallengeTask]:
    ...
    inst = _offtarget_instance() # off-target nomination task (data-gated on the fixture)
    if inst:
    ...
    trop = _delivery_tropism_instance() # delivery tropism task (label = approved-therapy registry)
    if trop:
...
def reference_submission() -> Submission:
    ...
    def predict(public_input: dict):
        ...
        if fam == "offtarget": # nominate + return the top-ranked candidate off-target
        ...
        if fam == "delivery": # return the serotype's grounded tissue (registry)
            from pen_stack.planner.delivery_predict import serotype_tropism
