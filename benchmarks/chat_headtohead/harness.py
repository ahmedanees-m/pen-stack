"""Head-to-head: the PEN-STACK chat vs ungrounded baselines.
...
not a fabrication). The differentiators:
  * provenance_labelled_rate - the chat labels every answer with its lane + provenance (general / literature-cited /
  ...
  * fabricated_stat_on_unsourceable - on a SPECIFIC unsourceable empirical genome-writing claim, did the system emit
    a confident specific number with no source? the chat abstains / redirects to the engine (0); the ungrounded LLM
    invents one.
  * false_grounding_rate - presented a general / unsourced fact AS a PEN-STACK-computed result. The chat: 0 by
...
# about a made-up entity (the chat should redirect/abstain; the ungrounded LLM tends to fabricate a statistic).
QUERIES = [
...
def run() -> dict:
    ...
    for q, kind in QUERIES:
        ...
        # a number is FABRICATED only if it is presented WITHOUT grounding. the chat's numbers on a grounded answer
    ...
    return {"n_queries": len(QUERIES), "n_unsourceable": n_unsourceable, "systems": summary, "rows": rows,
            "claim": "All three answer general questions; only the PEN-STACK chat labels every answer's provenance and "
