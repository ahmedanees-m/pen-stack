"""Verify-Bench: the verification-service gates as a reportable bench.

Reports the three verification gates from committed, deterministic code (no external data, CI-safe):
  1. rule spec parity: the exported machine-readable rule spec reproduces the live ruleset at 0 mismatches,
     every rule names a registered evaluator and carries a DOI or a note.
