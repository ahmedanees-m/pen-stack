# PA-WS1 Analysis A — durability axis vs ePRIDICT efficiency (distinctness)
...
scores existed). **Gate:** PA-G1 (Analysis A half).
...
**Honest caveat.** At n = 7,295 the correlation is *statistically* significant (p ≈ 1e-75): the two axes are
...
proceed to GSH-recovery (PA-WS3) + GEG-SH concordance (PA-WS2).
...
- **Single-timepoint durability** — `p_durable = 1 - P(silenced)` from a lower-quartile expression split, not a
  measured silencing-over-time trajectory (settled gate; see `_local_only/DEVIATIONS_AND_DISCLOSURES.md`).
