...
4. **Honest scope.** Tasks are bounded by the available documented writes (small, survivorship-biased). The
...
- **naive_baseline** — the honest baseline inside each scorer (safety-only, prevalence, Hamming).
- **llm_agent (PEN-Agent)** — `agent/orchestrator.py` + the grounded state machine `agent/pen_agent.py`,
...
note states, honestly, how many goals were LLM-driven vs fell back - the no-fabrication result holds either
