...
| llm_agent | 7 | = planner (grounded) | PASS | LLM orchestrator (nvidia) - LLM-driven on 2/4 goals; the rest gracefully fell back to the grounded tools; 19 grounded tool calls, 0 fabricated. Reaches the planner's numbers only by grounding every value. |

## Per-task results
| Task | Family | Available | Planner | Naive baseline | Gate |
|---|---|---|---|---|---|
| site_selection_blind_gsh | T1_site_selection | True | 0.7025 | 0.51 | - |
