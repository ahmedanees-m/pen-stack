"""Genome-Writing Bench v0.1 - solvers + leaderboard assembly.

Three solvers are compared on the same tasks:

  * `deterministic_planner` - the validated PEN-STACK planning tools (the reference).
  * `naive_baseline` - the baseline already implemented inside each scorer (safety-only site
...
def ungrounded_contrast_md(ung: dict | None) -> list[str]:
    ...
    lines = [
        ...
        "fabrication; an explicit refusal is deterministic. Two prompt conditions: **naive** (no anti-fabrication "
...
def robustness_contrast_md(bench: dict) -> list[str]:
    """Render the v0.3 ROBUSTNESS contrast: multi-write-type routing + adversarial probes, the
...
def render_leaderboard_md(bench: dict, llm: dict | None = None, ung: dict | None = None) -> str:
    ...
    lines = [
        ...
        "(unavailable = needs the atlas / Perry tables / an LLM, which run on the VM/local).",
    ...
    lines += [
        ...
        "No task is scored against a circular label.",
    ]
    return "\n".join(lines)
