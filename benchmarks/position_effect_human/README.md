...
held-out benchmark for the **v7.3.0 human K562 position-effect expression head** - the twin
...
1. **Feature *resolution*, not species, is the dominant variable.** On the **coarse 1 kb-binned atlas features the
   ...
   the 0.57 requires **exact-site feature extraction at serve time** (the v7.4 target). v7.3.0 makes the serve-time
...
| `SHA256SUMS` | integrity hashes (verify with `sha256sum -c SHA256SUMS`) |
| `README.md` | this file |
