# PEN-STACK: Scope and the edge of what it knows
...
Every agent output carries exactly one status, driven by the uncertainty-quantification signals (never hand-set):
...
| **not-computable** | no validated tool can ground it: the step refused, the query is out of scope (a known-unknown), or the agent abstained, returning a plain "I don't know" |
...
The unknown parts of the funnel stay **explicitly out of scope**, surfaced rather than faked. The scope matcher
...
| **long-term clinical durability** | the durability model is a *conditional chromatin-context* model (position effect on an integrated cassette), not a model of multi-year in-vivo persistence/clonal dynamics |
...
This boundary is made *legible* here; it is **not closed**. No amount of feeding equations closes it, for
anyone, today. As validated gold sets grow, the *calibrated* region widens; the out-of-scope list stays the
plain statement of what no tool here can ground.
