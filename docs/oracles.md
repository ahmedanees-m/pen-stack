...
PEN-STACK sits **on top of** the biomolecular foundation models. `pen_stack.oracles` wraps them under one
...
1. **A generative output is a candidate, never a claim.** `output_kind="candidate"` (Evo2 generation, ESM3,
   RFdiffusion, ProteinMPNN) → `as_claim()` **raises**. A candidate must pass writer-verification
...
| `energetics` | bridge off-target (gate ≥ 0.77) | claim |
...
grows with the cross-oracle spread); Principle 3.
