# The generative designer

PEN-STACK does not only *score* a design you give it; it *generates* candidate end-to-end writing
...
Each candidate is run through `verify()`, now **safety-gated**, legality-checked,
calibrated, and immune-profiled. A candidate **survives only if it is legal AND its safety
...
- `immune_profile`: the per-axis vector (`collapsed_score` stays `None`),
- `safety_decision`, `scope_flags`,
- `output_kind: "candidate"`: never asserted to work.

This generalises the `as_claim()` guard from single oracle outputs to whole designs. A novel writer
sequence in a candidate routes to writer-verification (critiqued, never claimed).

Pass an explicit `candidates=[...]` pool to discriminate a known set (atlas-independent); otherwise candidates
are enumerated by `candidate_space(goal)`: the validated planner (`plan_write`) crossed with every
...
`neg_immune_risk` is **grounded by the profile, not a placeholder**: it is the **worst-case per-axis
