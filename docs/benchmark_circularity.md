# The circularity in the recovery@k benchmark (and how it is fixed)

**Status:** acknowledged defect. A second reader should be able to
reproduce the argument from this document and the code alone.

## The defect

The "two-stratum recovery@k benchmark" (`pen_stack/validate/paper3_benchmark.py`) reported a
key result of **discriminating-stratum recovery@10 = 1.00 vs 0.00** for an intent-blind baseline, with a
...
search, not a confirmation. The non-circular signal therefore already lived in the control stratum; this release
promotes it to the primary result and strengthens it.

## The fix

- **Reframe intent-sensitivity as specification-compliance.** Keep the demonstration that the same
...
- **Blind safe-harbour site discovery (the new primary result).** Hold out literature-validated safe
...
- **Diversified writer recovery.** Add DSB-free large-cargo documented writes so the correct writer
...
- **Within-locus ranking.** Within a large safe-harbour gene, test whether the documented intronic
  safe bin ranks above other bins in the locus (`pen_stack/validate/within_locus_ranking.py`).
- **Consolidated report** with a scope statement per task; the primary result is the blind result, not
