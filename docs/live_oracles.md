...
PEN-STACK's foundation-model oracles can **actually execute**, not just defer. This release wires the live paths and
adds an **execution + latency** surface so the assistant tells you the cost *before* it runs anything.
...
| Arc STATE / scGPT | deferred | long_job | perturbation **outcome** needs the State-Transition model + a reference scRNA pipeline; deferred |
...
The cloud-A100 structure models are **held on purpose** (see the release notes): they can be run via these same
