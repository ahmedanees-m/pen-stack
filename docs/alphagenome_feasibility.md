# AlphaGenome feasibility + integration-mode decision (WS-C0)
...
| Mouse support | `MUS_MUSCULUS` works; **ES-Bruce4 RNA-seq (`EFO:0005483`)** is an exact match to the TRIP supervision cell line - used by the WS-B1 endogenous-expression baseline. |
...
- **AlphaGenome invoked on-demand per locus** for: (a) the endogenous-expression durability baseline (WS-B1,
  done); (b) predicted-vs-measured track validation (C2); (c) 3D contact-map deltas at candidate sites (C3);
...
(100-500 KB) context. Not required for M1 - the measured-track atlas is the backbone.
...
- Cross-cell-type writability claims are **bounded by AlphaGenome's training coverage** (C2 scope note in M1).
- 3D structural risk ships as a **flag with confidence, never a hard pass/fail** (Gate G-C); it is a
