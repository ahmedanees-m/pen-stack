# Delivery immunology: the safety↔efficacy balance

PEN-STACK scores and constrains the **whole** delivery palette (8 vehicles), not just dual-AAV. Later work added
...
> **The invariant (unchanged across the program).** The in-vivo immune *magnitude*, how strongly a
...
- a summary `safety_score = min(immune_score, genotox_score)` (precautionary worst-axis);
...
immunogenicity (non-integrating, but NAb-limited), and **lentivirus** on genotoxicity (a highly efficacious
...
## Completion & calibration: anti-PEG, proxy labelling, and the unified profile

This release finishes the picture (`docs` + `pen_stack/planner/{antipeg_oracle,immune_profile}.py`,
`pen_stack/validate/immune_calibration.py`):

- **Anti-PEG axis.** `antipeg_oracle.py` + `configs/antipeg.yaml`. Pre-existing/induced anti-PEG
  antibodies gate **re-dosing** of PEGylated LNP. Same serosurvey pattern as before: a population
...
- **Proxy calibration.** `immune_calibration.py`. `calibrate_axis()` computes a Spearman ρ +
  ...
  dataset exists, **every axis is currently labelled a mechanistic/population proxy**; that label is
...
- **Unified immune-risk profile.** `immune_profile.py`, exposed as **`Verdict.immune_profile`**.
...
- **Route/privilege modifier.** A documented qualitative **route/immune-privilege modifier** (eye/CNS materially lower realized
...
| Axis | Risk kind | before | Now | Grounding |
|---|---|---|---|---|
| **Genotoxicity** | permanent / oncogenic | tier | **computed** | integration-site catalogues × oncogene loci |
| **Adaptive (CD8 T-cell)** | reversible | tier | **computed** | MHC-I epitope load over the capsid sequence |
| **Innate** | reversible | tier | **computed** | CpG / dsRNA motif load of the cargo sequence |
| **Pre-existing / NAb (B-cell)** | reversible (eligibility) | tier | **data-grounded** | published serosurveys |
| **Anti-PEG (LNP re-dosing)** | reversible (re-dosing) | n/a | **data-grounded** | anti-PEG serosurveys |
| **Efficacy** | n/a | tier | documented | construct/context-specific by nature |

Each computed signal answers through the `OracleResult` contract (value + provenance + native uncertainty
...
### Genotoxicity: `genotoxicity_oracle.py`
...
- **Lentiviral (HIV) 2.08×** enrichment (n = 88,743, robust) vs **gammaretroviral (MLV) 5.65×** (the LMO2 /
  ...
  ordering **from data**. The computed lentivirus score (0.48) **validates** the documented "moderate"
  tier (0.5).
...
### Adaptive / CD8: `capsid_epitope_oracle.py`
...
### Pre-existing / neutralizing antibody: `seroprevalence_oracle.py`
...
Across this work the delivery immune/safety profile moved from documented `low/moderate/high` tiers to **real
