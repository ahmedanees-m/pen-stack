...
## v5.6 completion & calibration: anti-PEG, proxy labelling, and the unified profile

v5.6 finishes the picture (`docs` + `pen_stack/planner/{antipeg_oracle,immune_profile}.py`,
...
- **Anti-PEG axis.** `antipeg_oracle.py` + `configs/antipeg.yaml`. Pre-existing/induced anti-PEG
  antibodies gate **re-dosing** of PEGylated LNP. Same serosurvey pattern as v5.5: a population
...
| **Anti-PEG (LNP re-dosing)** | reversible (re-dosing) | n/a | **data-grounded** (v5.6) | anti-PEG serosurveys |
...
### Genotoxicity: `genotoxicity_oracle.py` (v5.2)
...
### Adaptive / CD8: `capsid_epitope_oracle.py` (v5.3)
...
### Pre-existing / neutralizing antibody: `seroprevalence_oracle.py` (v5.5)
