# Calibrated uncertainty in PEN-STACK
...
The design principle: **wrap, don't retrain.** PEN-STACK takes the existing LightGBM heads (safety,
...
The bridge off-target ranker reports a *ranker-calibrated* band (held-out AUROC; **0.88** with the
...
clinical durability, higher-order epistasis) stays explicitly out of scope. The calibration makes that boundary
