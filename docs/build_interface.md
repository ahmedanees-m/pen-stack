...
PEN-STACK reaches the **digital→physical** interface, responsibly. A cleared, legal design becomes a
runnable protocol DRAFT carrying its immune-risk profile (refused outright if the safety gate flags it);
a typed ingestion API returns experimental results as candidate evidence admitted only through the
...
the immune profile and full provenance in the metadata. Protocols are drafts; nothing is auto-run.
...
Candidate**. The only way it enters the curated world-model is the gate (`gate_admit`): automated checks
**and** explicit human approval. No process auto-edits curated truth (Principle 1). Immune-measurement results can
begin validating the proxies on a later pass.
...
`run_simulated` executes a protocol in silico: it samples an "observed" readout from the twin + measurement
noise, **labelled `SIMULATED`**. This lets the closed loop run end-to-end **export → sim → ingest** without
