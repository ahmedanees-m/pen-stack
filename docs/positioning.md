...
off-target engine). This release adds the missing **evaluation layer**.
...
- **Bounded scope.** Tasks are bounded by the available documented writes (small, survivorship-biased). The
...
- **M1 - Writable Genome, hardened (B,C,D,F).** Strong baselines (endogenous-expression, multi-mark,
...
- **M2 - the Genome-Writing Bench + PEN-Agent (E).** The benchmark and a grounded agent. Framed
...
- **M3 - Multiplex + guide QC (G).** A translocation-risk screen and a bridge-RNA guide ranker.
