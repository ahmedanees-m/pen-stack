...
PEN-STACK turns *"I'm uncertain"* into *"run **this** experiment next."* It reads the calibrated
twin's uncertainty and the immune-risk labels, scores each candidate experiment by the information it is
...
- **`immune_voi(candidate)`**: value of information for **validating an immune PROXY axis**: an axis still
...
- **`acquisition_score`** = `w_eig·EIG + w_unc·entropy + w_imm·immune_voi`. Fully traceable to twin quantities +
  labels; deterministic given inputs (no fabricated values).
...
The experiment designer is only as good as the twin and the labels it queries. Its advantage is
...
them**; prospective benefit awaits a lab partner.
