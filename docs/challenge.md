...
PEN-STACK's accumulated bench tasks become an **open, recurring, held-out** benchmark others build
...
- the private label is computed by the **validated PEN-STACK layers** (the rules, the Guardian, the
  immune profile), so **no task uses a circular label**; a fabricated answer cannot match an invented label
  and simply scores 0;
...
- task families generalise the internal bench: `legality`, `safety`, and an **immune-risk** task grounded in the
  oracles (future rounds add write-types, adversarial, outcome, experiment-design, closed-loop).
...
- **Immune-risk first-class**: an immune-risk task draws directly on the profile.
