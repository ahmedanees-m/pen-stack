# The PEN-STACK Verifier (`verify(design) → Verdict`)
...
- **`confidence`**: calibrated trust in the **soft/learned** components (the L4 layer), with an
...
- **Deferral, not guessing.** Unsupported write types defer (`legal: null`), and out-of-scope questions
