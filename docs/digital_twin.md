...
PEN-STACK can predict *what the cell does after the write*: calibrated, OOD-gated, and scope-bounded.
It computes what mechanism allows, adds an in-distribution virtual-cell estimate, screens immune outcome from the
immune profile, and states its boundary at phenotype plainly. The twin is a **hypothesis engine, not an oracle of
truth**.
...
o["immune_outcome"]      # the per-axis profile (sourced, not invented)
...
`predict_response(cell_state, perturbation, model="state")` wraps Arc **STATE** / **scGPT** under the
...
virtual-cell response (when available) + the immune profile. The interval **widens under OOD** rather than
...
The twin predicts what mechanism computes, what an in-distribution virtual-cell model supports, and what the
