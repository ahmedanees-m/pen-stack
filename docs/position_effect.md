...
Previously this was a closed-form **heuristic** with a heuristic ±0.20 band that the code itself labelled *"NOT
a trained conformal interval"*, and which **failed** independent validation (ρ=0.12 vs Damdindorj 2014). This release
...
| Metric | cassette-only | context-only (v3.x durability head) | **factored model** | Δ vs head (CI) |
|---|---|---|---|---|
...
- **Without a context (or artifact)** → the closed-form heuristic band, exactly as before; **backward compatible**
  (the relative-scale contract and all prior tests are intact).
...
- **Public data cannot earn a validated-axis grade** (the wall). This release ships the learned+calibrated upgrade + the
