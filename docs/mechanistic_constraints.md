# Mechanistic constraints in PEN-STACK

The funnel rejects ideas the physics rules out. This layer extends the computable filters **only where the mechanism
...
`validate/offtarget_energetics_eval.py`. This is the only place PEN-STACK adds mechanism to a model rather than
