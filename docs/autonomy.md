# Autonomy levels: PEN-STACK stops at Level 3

PEN-STACK's closed loop is a **Level-3** design→build→test→learn engine: closed, but with **humans/lab in
...
| 1 | Assisted: tools score/check, human decides each step | verifier |
| 2 | Partial: the system proposes designs + experiments; human runs + decides | - |
| **3** | **Closed loop with human control at every gate**: the system runs the full cycle, but pauses for human approval at safety, build, and belief-admission, and flags anomalies | **here** |
...
2. **Human in control at every gate.** The loop pauses for the `approver` at **safety** (export is refused for a
   ...
   enter the curated world-model only via the gate with explicit approval).
