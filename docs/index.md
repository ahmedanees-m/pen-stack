...
| **Write Planner** (`pen_stack.planner`) | Find the optimal site x writer x cargo x delivery for a goal |
...
pen-stack monitor --back-test        # living-database scan
```
...
are carried forward; circular or unvalidatable claims were retired (see the program document).
