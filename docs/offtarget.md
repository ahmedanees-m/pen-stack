# The off-target nomination engine
...
- **Nuclease (Cas9):** given a guide + candidate sites (e.g. from a Cas-OFFinder scan), each candidate gets a
  ...
  **real CRISOT-Score** when the (guide, site) is in the cached bench, and a documented **chromatin-accessibility
  ...
  controls) found that with a **cell-type-matched** track (ENCODE HEK293T DNase) accessibility predicts
  ...
  annotation and is **intentionally not folded into the numeric risk score** (the incremental analysis
