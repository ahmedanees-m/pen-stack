...
This layer matures the reasoning on top of the verifier, the environment, the oracle mesh,
and the living world-model. Give it a goal and an intent and it returns a small set of
...
| **Self-critique / revise** | `critique_and_revise(design)` | the critic only flags + suggests a design-level swap (never invents a number); the revision is **re-verified**; falsifiable, in that it improves flawed plans (illegal→legal) and never spuriously touches clean ones (`critique_falsifiability()`) |
