...
population (anti-vector + anti-PEG seroprevalence). This module tests, two-sided and honestly (the v3.4
...
HONESTY (the central point of this workstream). Public, citable, paired (proxy, observed-immunogenicity-
...
calls an axis "validated" when the CI excludes zero); with insufficient data it returns the honest
"mechanistic_proxy" label. The current per-axis status (`AXIS_STATUS`) is therefore *proxy* for every axis —
that honest label is the deliverable, and it travels with the unified immune profile (v5.6 WS-PROFILE).
"""
...
_MIN_N = 6                     # below this, too few paired points to validate (honest abstention)
...
# Current honest status of each real axis. No axis has a sufficient public paired (proxy, observed-outcome)
...
def run(out: str | Path = "phase_5.6/out/immune_calibration.json") -> dict:
    """Emit the honest per-axis calibration report. With no sufficient public outcome dataset, every axis is a
