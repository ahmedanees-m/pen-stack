...
Runs the hazard screens over a design and returns typed, provenanced hits. Three+ screens:
  ...
  * sequence_homology — delegated to a wrapped external screener when enabled (baseline: no-op + honest note).
...
def screen_design(design: dict, registry=None) -> list[ScreenHit]:
    ...
    if seq:
        hits += reg.sequence_homology(seq)        # external screener when enabled, else honest no-op
    return hits
