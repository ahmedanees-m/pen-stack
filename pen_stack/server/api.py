...
def verify_endpoint(design: dict):
    """v3.3 verification service: submit a proposed genomic write, get back a structured Verdict,
...
def verify_proof_endpoint(design: dict):
    """v6.12 verification service: the repair-oriented proof object. Returns the three axes
...
def graph_query_endpoint(q: dict):
    """v4.5 world-model graph: multi-hop query. Body: {locus, cargo_form?}. Returns writers that
...
def safety_endpoint(design: dict):
    ...
    v7.3.8 additive: also carries ``standards``, the SafetyVerdict expressed in the community-standard
...
@app.get("/safety/concordance", tags=["v6.1 AI surface"])
def safety_concordance_endpoint():
    """v6.12 standards concordance: run the Guardian over the committed labelled probe set
...
def campaign_endpoint():
    """v7.0: the validation-campaign engine. Returns the expression-validation campaign: the next batch of
...
def cloudlab_endpoint(req: dict):
    """v7.0: safety-gated cloud-lab submission. Body: {design, experiment?, provider?, actor?}. The
...
def brains_endpoint():
    """v7.0: benchmark the EIG/VOI experiment designer against the public SDL optimizers (BayBE / Atlas),
...
def writespec_endpoint(req: dict):
    """v6.14: parse a plain-language genome-writing request into a typed, ontology-backed WriteSpec.
...
def oracle_affinity_endpoint(req: dict):
    """v6.13 protein-ligand binding-affinity (Boltz-2 head) under the oracle contract. Body:
...
def delivery_endpoint(req: dict):
    """v6.11 cross-modality delivery recommender. Body: {cargo_form, cargo_bp?, target_tissue?,
...
@app.post("/capsid/generate", tags=["v6.11 delivery"])
def capsid_generate_endpoint(req: dict):
    """v6.11 verify-gated generative AAV capsid candidates: propose VP1 555-595 variants of a WT, score each
...
def delivery_tropism_endpoint(serotype: str | None = None, target_tissue: str | None = None):
    """v6.11 grounded AAV serotype<->tissue tropism priors (approved therapies). Pass a `serotype` for the tissue(s)
...
@app.get("/twin/promoters", tags=["v6.1 AI surface"])
def promoters_endpoint():
    """v7.3.10: the selectable promoter palette (constitutive + tissue-specific, literature strength/context/DOI).
