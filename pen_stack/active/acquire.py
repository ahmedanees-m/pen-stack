...
def _effective_sd(outcome: dict, cell_state: str) -> float:
    ...
    the base SD by those real signals so EIG reflects them (v7.3.18.1: fixes EIG reading a constant ~0.02 for
...
def immune_voi(candidate: dict, cell_state: str = "") -> float:
    """Value of information for validating an immune PROXY axis (v5.6): an IN-SCOPE axis still labelled a proxy
    that this experiment would MEASURE is high-VOI (turns proxy -> outcome-validated). Reads the v5.6 validation
    labels. v7.3.18: requires the axis to be IN SCOPE, an abstaining axis (e.g. anti-PEG on a non-PEG vehicle,
...
# --- design-specific informativeness signals (v7.3.18): the mechanistic twin's interval is a fixed heuristic band,
...
def acquisition_components(candidate: dict, cell_state: str, model_ctx: dict | None = None,
    ...
    (twin uncertainty / v5.6 proxy labels / capacity rule / measured-atlas roster); the composite is
    feasibility-gated. Returned so the UI can show WHY one experiment out-ranks another, not just a number.

    v7.3.18.1: EIG now already folds cell-type coverage + OOD into its effective uncertainty (see `_effective_sd`),
...
def acquisition_score(candidate: dict, cell_state: str, model_ctx: dict | None = None,
    ...
    """Composite acquisition (feasibility-gated information value). Fully traceable to twin quantities + v5.6
