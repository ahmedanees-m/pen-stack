"""Mesh-upgraded atlas features + honest blind re-validation (v4.0, WS-ATLAS / A1).
...
reported honestly whatever it is (improvement, parity, or regression), never hidden.
"""
...
def revalidate_atlas(in_distribution: bool = True) -> dict[str, Any]:
    ...
    Honest by construction: when the oracle backends are deferred, no new features are added, so the atlas is
    ...
    # the SHA-locked v3.1 blind-GSH baseline (README / paper1): AUROC 0.68, CI 0.54-0.83, N small (honest)
    v3x_auroc = 0.68
