...
GSH/TRIP gold sets are small) yields **wide** intervals - that width is an honest output, reported with N,
...
def conformal_quantile(scores: np.ndarray, alpha: float) -> float:
    ...
    calibration points to guarantee the requested coverage - an honest 'cannot certify' rather than a
    falsely tight bound)."""
    ...
    if k > n:
        return float("inf")            # cannot certify this coverage at this N (honest)
    return float(s[k - 1])
...
class ConformalClassifier:
    ...
    honest choice under class imbalance (genotoxic sites are rare).
