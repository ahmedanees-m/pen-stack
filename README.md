...
```
pen-stack/
  README.md  LICENSE  CITATION.cff  CHANGELOG.md      project front matter (MIT; cite-this-repository)
...
Version 8.0.5 is the current stable release; 8.0.0 established the committed public API and the 8.0.x patches
(8.0.1 through 8.0.5) are reproducibility, documentation, safety, packaging, readability, and API-hardening fixes
within it (no signature changes). The committed public API is the SDK
...
may change as measured data becomes available. The major version marks this API commitment; it consolidates
the gated 7.x development history rather than replacing it, so every pre-registration, sealed benchmark, and
reported negative in that history stays intact and citable.
