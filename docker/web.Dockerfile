# PEN-STACK Web Platform image (one container: built frontend + the FastAPI gateway over the engine).
...
# the AlphaGenome client so the hosted variant-effect oracle is callable when keys + PEN_STACK_ORACLE_NET=1
