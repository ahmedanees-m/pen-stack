...
const CARGO_MAX = 300000;  // matches the backend /plan cargo_bp bound (ge=1, le=300000)
// fallback if /api/celltypes is unreachable (keeps the page's coverage accurate either way)
...
export default function SiteFinder() {
  ...
  const [cargoNote, setCargoNote] = useState(null);
  ...
  const [planError, setPlanError] = useState(null);  // a plan-only failure (surfaced, not silently hidden)
  const [reach, setReach] = useState(null);  // crosslink: which writer families are grounded-reachable (Tier-1) here
  ...
  function setCargoSafe(v) {
    ...
    if (Number.isNaN(n)) { setCargo(1); setCargoNote(null); return; }
    const clamped = Math.min(CARGO_MAX, Math.max(1, n));  // real payload range [1, CARGO_MAX]
    setCargo(clamped);
    setCargoNote(clamped !== n ? `clamped to ${clamped.toLocaleString()} bp (allowed 1–${CARGO_MAX.toLocaleString()})` : null);
  }

  async function run() {
    if (!geneOk) return;
    setBusy(true); setError(null); setPlanError(null); setLoci(null); setPlans(null); setReach(null);
    ...
    try {
      ...
      if (w.status === "fulfilled") {
        const lociArr = w.value.loci || [];
        setLoci(lociArr);
        // crosslink: which writer families can actually reach the top locus (grounded, Tier-1 only)
        if (lociArr[0]) api.crosslinkWriters(lociArr[0].chrom, lociArr[0].bin, ct).then(setReach).catch(() => setReach(null));
      }
      if (p.status === "fulfilled") setPlans(p.value.plans || []);
      else setPlanError(p.reason);  // surface a plan-only failure (e.g. 422 unknown intent) instead of hiding it
  ...
  return (
    <div className="space-y-4">
      <WhatYouGet
        intro="Find the best places to write into a gene, every candidate 1-kb locus scored for genomic safety and expression durability, plus ready-to-use writer + site plans for your cargo and edit intent."
        features={[
          { name: "Writable loci", tells: "the scored 1-kb bins in and around your gene, ranked", output: "table · safety / durability / writability (0–1)" },
          { name: "Site safety", tells: "is the locus away from cancer & essential genes?", output: "0–1 · higher = safer" },
          { name: "Durability", tells: "will an insert here keep expressing (resist silencing)?", output: "0–1 · higher = lasts" },
          { name: "Writability", tells: "overall write-suitability (0.5·safety + 0.5·durability)", output: "0–1 · the headline" },
          { name: "Ranked write plans", tells: "concrete writer × site combos for your cargo + intent", output: "ranked list · genotoxic sites flagged" },
          { name: "Cell-type coverage", tells: "which cell types have a measured atlas (refusal otherwise)", output: "K562/HepG2 full · HSPC partial · others disabled" },
        ]}
        example="Enter CCR5 in K562 → 7 candidate loci with safety / durability / writability, plus ranked bridge-IS110 write plans sized to your cargo. Pick an oncogene like LMO2 and the unsafe sites are flagged genotoxic, not hidden."
      />
      <Card title="Find writable sites" subtitle="Loci scored by safety and durability for a target gene in a chosen cell type.">
        <div className="grid gap-3 sm:grid-cols-4">
          ...
          <Field label="Cargo bp" hint={`for the ranked plans (capacity/delivery) · 1–${CARGO_MAX.toLocaleString()} bp`}>
            <input className="input" type="number" min={1} max={CARGO_MAX} step={100} value={cargo}
                   onChange={(e) => setCargoSafe(e.target.value)} />
            {cargoNote && <p className="mt-1 text-[11px] text-amber-300/90">{cargoNote}</p>}
          </Field>
        </div>
      ...
      {loci && result && (
        <Card title={`Writable loci for ${result.gene}`}
          ...
          {reach && Object.keys(reach.families || {}).length > 0 && (
            <div className="mt-4 rounded-lg border border-brand/25 bg-brand/5 p-3">
              <div className="text-[11px] uppercase tracking-wide text-fg-faint">
                Writers that can reach {reach.locus} · grounded (Tier-1 scannable)
              </div>
              <div className="mt-2 flex flex-wrap gap-2">
                {Object.entries(reach.families).map(([fam, n]) => (
                  <Badge key={fam} tone="brand">{fam.replace(/_/g, " ")} · {Number(n).toLocaleString()} systems</Badge>
                ))}
              </div>
              <p className="mt-2 text-[11px] leading-relaxed text-fg-faint">
                Only Tier-1 “scannable” writers, target-programmable to any genomic address, are shown as
                grounded-reachable here ({Number(reach.n_systems).toLocaleString()} systems total). Tier-2/3 families
                (context-dependent or not reliably targetable) are candidates, not listed as reachable. Reachability is a
                property of the writer mechanism, so it is the same at every locus.
              </p>
            </div>
          )}
        </Card>
      ...
      {planError && result && !plans && (
        <Card title="Ranked write plans">
          <ErrorNote error={planError} />
          <p className="mt-2 text-[11px] text-fg-faint">The loci table above still scored; only the ranked-plan step failed for this input (e.g. an unsupported edit intent). Pick a different edit intent, or check the cargo size.</p>
        </Card>
      )}
        ...
        <Card title="Ranked write plans" subtitle={`Traceable plans for "${intent.replace(/_/g, " ")}" · cargo ${cargo} bp · ${result.ctLabel}`}>
          {plans.length === 0 ? <p className="text-sm text-fg-faint">No plans returned.</p> : (
            <ol className="space-y-2">
              {plans.map((p, i) => (
                <li key={i} className={`rounded-lg border bg-ink-900 p-3 text-sm ${p.genotoxic ? "border-bad/50" : "border-line"}`}>
                  <div className="flex items-center justify-between">
                    <span className="font-medium">{i + 1}. {p.writer} → {p.site?.chrom}:bin{p.site?.bin}
                      {p.genotoxic && <Badge tone="bad">genotoxic · oncogene/essential-gene zone</Badge>}</span>
                  ...
                  <div className="mt-1 flex flex-wrap gap-3 text-[11px] text-fg-dim">
                    <span className={p.genotoxic ? "text-bad" : undefined}>safety {num(p.safety)}</span><span>durability {num(p.durability)}</span>
                  ...
                  {p.genotoxic && <p className="mt-1 text-[11px] text-bad/90">Low safety ({num(p.safety)}): this locus sits in an oncogene/essential-gene danger zone. Surfaced because it matches your on-target intent, but it carries insertional-oncogenesis risk, not a safe-harbour recommendation.</p>}
                </li>
              ))}
            </ol>
          )}
