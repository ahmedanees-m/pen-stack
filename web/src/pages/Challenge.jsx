...
export default function Challenge() {
  ...
  return (
    <div className="space-y-4">
      <WhatYouGet
        intro="The open, held-out Genome-Writing Challenge: the public benchmark tasks and the PEN-STACK reference scores, so anyone can compare genome-writing predictions on a common yardstick."
        features={[
          { name: "Public tasks", tells: "the held-out benchmark tasks (site selection, off-target, …)", output: "task list + the metric each is scored on" },
          { name: "Reference leaderboard", tells: "the PEN-STACK reference score per task", output: "AUROC / metric + confidence interval" },
          { name: "Scoring rule", tells: "how submissions are judged", output: "offline, against held-out wet-lab labels (never over HTTP)" },
        ]}
        example="Open the blind safe-harbour-discovery task → its held-out metric and the reference AUROC with its bootstrap confidence interval; labels are measured wet-lab calls, not submitter claims."
      />
      <ScoreGuide
