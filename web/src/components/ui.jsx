...
import { invalidNucleotides } from "../lib/format.js";

// A reusable non-nucleotide warning shown under a sequence input (flags junk input rather than
// silently accept characters outside A/C/G/T/U/N). Renders nothing when the sequence is clean or empty.
export function SeqWarning({ seq }) {
  const bad = invalidNucleotides(seq);
  if (!bad.length) return null;
  return <p className="mt-1 text-[11px]" style={{ color: "var(--warn)" }}>⚠ non-nucleotide character(s): {bad.join(" ")}, expected A/C/G/T.</p>;
}
