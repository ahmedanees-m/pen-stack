...
export const api = {
  ...
  // v7.0 closed loop: the validation campaign, the SDL-brain benchmark, safety-gated cloud-lab submission,
  ...
  // v6.11 learned AAV capsid packaging-fitness (FLIP-AAV); AAV-only, abstains for other vectors
  capsidFitness: ({ vp1_sequence, vector = "AAV" }) =>
    req("/api/capsid_fitness", { method: "POST", body: { vp1_sequence, vector } }),
  // v6.11 verify-gated generative capsid candidates: propose VP1 variants, keep survivors with fitness >= WT
  capsidGenerate: ({ wt_vp1, n = 200, max_mut = 4, top = 20 }) =>
    req("/api/capsid/generate", { method: "POST", body: { wt_vp1, n, max_mut, top } }),
  // v6.11 grounded AAV serotype<->tissue tropism priors (FDA/EMA-approved therapies; known-unknown for novel capsids)
