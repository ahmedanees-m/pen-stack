"""Build the v6.8 writer-efficiency artifacts.

Builds the curated dataset parquet, runs the held-out-family + held-out-locus evaluation vs the KB family-mean
baseline (the acceptance gate), calibrates a family-blocked split-conformal interval, fits the final model, and
writes:
...
def main() -> None:
    ...
    print(f"\nACCEPTANCE GATE: {rep['gate_C_G2']['verdict']}")
