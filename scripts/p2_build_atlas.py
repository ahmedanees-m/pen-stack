"""Atlas pipeline (reproducible): expand -> mechanism -> therapeutic readiness.

Runs the three atlas-building steps in order so ``atlas.parquet`` carries every atlas column. Network
is needed only for the UniProt expansion step (cached under data/external/atlas after the first run).
