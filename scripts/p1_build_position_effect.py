"""Build the position-effect model + trained-conformal artifact.
...
Every number is from a real CV run on real TRIP supervision. No fabrication. The learned-model gate is evaluated and
printed; the model is still saved (the serving layer decides whether to *serve* it based on the gate, see twin/outcome.py).
"""
...
def main() -> None:
    ...
    gate = {"beats_durability_head_expression": bool(beats_expr),
            ...
            "verdict": ("learned model beats the durability head, serve it behind the serving layer"
                        if (beats_expr or beats_sil) else
                        "learned model does NOT beat the durability head at this N, retain head/heuristic, "
                        "report negative; the value rests on trained-conformal intervals + TPE-Bench")}
    print(f"\nLEARNED-MODEL GATE: {gate['verdict']}")
