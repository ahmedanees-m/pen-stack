"""Consolidated report - durability baselines + the safety primary-metric switch.
...
def build() -> str:
    ...
    lines = [
        "# PEN-STACK v3.1 - Durability baselines + safety primary-metric switch",
        "",
        "Baselines around the durability and safety claims: a same-cell-line endogenous-expression "
        ...
        f"- Scope: the absolute CI is wide ({gsh.get('n_positives')} curated positives); a larger "
    ...
    if b1.get("available"):
        lines += [
            ...
            f"{b1.get('ci_note', '')}",
            f"- **Finding: {b1.get('honest_finding', 'n/a')}.** The point estimate clears the "
            ...
            "with an non-significant CI, not as a proven gain.",
