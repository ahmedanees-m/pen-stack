"""Reproducible build (v6.11): train the AAV capsid-fitness model on the FLIP-AAV benchmark and report the
...
def main() -> dict:
    ...
    for split in ["sampled", "mut_des"]:
        ...
        if split == "mut_des":
            ...
            # reference distribution for a percentile readout: the model's OWN predicted fitness over ALL measured
            # FLIP-AAV variants in the library (representative, not just the designed test split). 101 percentiles.
            full_pred = m.predict(np.vstack([onehot(w) for w in d["w"]]))
            pred_pct = [round(float(np.percentile(full_pred, p)), 4) for p in range(101)]
            pickle.dump({"model": m, "window": [W0, W1], "consensus": cons, "aas": AAS,
                         "pred_percentiles": pred_pct, "n_ref": int(len(full_pred)),
